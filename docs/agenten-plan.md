...
|---|---|---|---|
| Claude (fertig) | keine, Shell-Zeile `"$KADRELL" hook claude` | kein Eintrag: `--settings '<json>'` beim Start jedes Prozesses (`ClaudeHook.launchArgs`) | `SessionStart` → session-id; `UserPromptSubmit` → working; `PermissionRequest`, `Notification` (permission_prompt/idle_prompt/agent_needs_input/elicitation_dialog) → waiting; `Stop` (`last_assistant_message`) → idle |
